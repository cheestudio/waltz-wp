<?php
/* Template Name: Campaign (Clients)
========================================================= */ ?>

<?php
if (!defined('ABSPATH')) exit;
get_header();
?>


<section class="campaign-hero current-clients">
  <div class="row campaign-hero-grid">
    <div class="grid-entry number-entry animated-element" data-anim-y="-30">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-01.svg" alt="">
    </div>
    <div class="grid-entry">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-02.svg" alt="">
    </div>
    <div class="grid-entry number-entry animated-element" data-anim-x="30" data-anim-delay="0.7">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-03.svg" alt="">
    </div>
    <div class="grid-entry diamond-entry">
      <img class="diamond" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-04.svg" alt="">
      <img class="diamond-animated" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-04-animated.svg" alt="">
    </div>
  </div>
  <div class="row campaign-hero-grid">
    <div class="grid-entry">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-05.svg" alt="">
    </div>
    <div class="grid-entry number-entry animated-element" data-anim-scale="0.5" data-anim-delay="0.3">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-06.svg" alt="">
    </div>
    <div class="grid-entry">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-07.svg" alt="">
    </div>
    <div class="grid-entry number-entry animated-element" data-anim-scale="1.1" data-anim-delay="1">
      <img src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/client-hero-08.svg" alt="">
    </div>
  </div>
</section>

<section class="campaign-countdown">
  <div class="container">
    <div class="primary">
      <h2>We’re counting down to our 20th Anniversary!</h2>
      <p>As we celebrate this exciting milestone, we reflect on a journey defined by groundbreaking projects and lasting partnerships. We are grateful for our incredible team and visionary clients who are the heartbeat of this agency. Together, we've turned dreams into reality, setting the stage for a future filled with even greater achievements. Cheers to another 20 years filled with boundless possibilities and fueled by the spirit of creativity!</p>
      <div class="button-wrap">
        <a class="get-started-top-link" href="#" onclick="window.open('#contact-us', '_blank');return false;">Let's Get Started</a>
      </div>
    </div>
    <div class="secondary">
      <?php get_template_part('partials/elements/campaign--countdown'); ?>
      <img class="countdown-accent" data-parallax="-0.8" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/Texture-Dots-B.svg" alt="" aria-hidden="true">
      <img class="countdown-20" data-parallax="-1" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/20-year-mark.svg" alt="20 Years of Waltz">
    </div>
  </div>
</section>

<?php get_template_part('partials/elements/campaign--services-grid', null, [
  'class' => 'current-clients'
]); ?>

<section class="campaign-split reverse">
  <div class="container">
    <div class="primary">
      <h2>Supporting Roles & Headlining Acts</h2>
      <p>Waltz Creative has proudly served businesses and nonprofits, helping them shine brighter and perform better. Drawing from a diverse range of specialties, styles and influences, our talented team consistently delivers the innovative strategies, unexpected creative work and unmatched service to drive lasting success.</p>
      <p>We leverge our expertise to help you stand out, move your brand forward and generate leads. Let Waltz guide you to more clicks, better engagement, greater sales and captivating creative that moves your audience.</p>
      <p>Stronger together, we collaborate enthusiastically to find the right approach for each client.</p>
      <div class="button-wrap">
        <a class="waltz-this-way-services" href="https://waltzcreative.com/services/" title="Learn more about Waltz Creative" target="_blank">Waltz This Way</a>
      </div>
    </div>
    <div class="secondary">
      <?php get_template_part('partials/elements/campaign--homepage'); ?>
    </div>
  </div>
</section>

<section class="campaign-form" id="contact-us">
  <div class="container">
    <div class="inner">
      <h2>Let's Dance</h2>
      <?php gravity_form(11, false, false, false, '', true); ?>
      <div class="graphic-group">
        <img class="sunburst" data-parallax="0.5" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/Sunburst.svg" alt="" aria-hidden="true">
        <img class="year-stacked" data-parallax="-0.5" src="<?php bloginfo('template_directory'); ?>/assets/img/campaign/2024-Graphic.svg" alt="" aria-hidden="true">
      </div>
    </div>
  </div>
</section>

<script type="text/javascript">
  function sendClickEvent(event) {
    gtag('event', 'click', {
      'event_category': 'Current Client Clicks',
      'event_label': event.target.className,
      'value': 1
    });
  }
  document.addEventListener('click', sendClickEvent);
</script>

<?php get_footer(); ?>